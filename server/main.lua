        ...
        if price <= currentMoney then
            MySQL.Async.execute("UPDATE market_items SET stock=@stock WHERE marketid=@location AND items=@item", {['@stock'] = stock, ['@location'] = location, ['@item'] = item}, function(count)
                if count > 0 then
                    ...
                    TriggerClientEvent('ox_lib:notify', source, {title = Lang:t('success.buy_prod'), description = amount.."x "..RSGCore.Shared.Items[item].label, type = 'success', duration = 5000 })
                end
            end)
        else 
            TriggerClientEvent('ox_lib:notify', source, {title = 'Error', description = Lang:t('error.player_no_money'), type = 'error', duration = 5000 })
        end
    end)
end)
...
AddEventHandler("rsg-vendor:server:vendorInvReFill", function(location, item, qt, amount)
    ...
    exports.oxmysql:execute('SELECT * FROM market_items WHERE marketid = ? AND items = ?',{location, itemv} , function(result)
        ...
        TriggerClientEvent('ox_lib:notify', source, {title = 'Success', description = Lang:t('success.refill').." " ..qt.. "x " ..item.label, type = 'success', duration = 5000 })
    end)
end)
    ...
    exports.oxmysql:execute('SELECT * FROM market_owner WHERE marketid = ? AND owned = 0',{data.location} , function(result)
        if result[1] ~= nil then
            if Player.Functions.RemoveMoney("cash", data.price, "stall-bought") then
                ...
                TriggerClientEvent('ox_lib:notify', source, {title = 'Success', description = Lang:t('success.buy_t'), type = 'success', duration = 5000 })
        ...
        if result[1] ~= nil then
            MySQL.Async.fetchAll("SELECT * FROM market_owner WHERE citizenid=@citizenid AND owned=1 AND marketid=@marketid", { ['@marketid'] = location, ['@citizenid'] = Playercid }, function(result2)
                if result2[1] ~= nil then
                    ...
                    TriggerClientEvent('ox_lib:notify', source, {title = 'Success', description = Lang:t('success.transfert_t'), type = 'success', duration = 5000 })
                else
                    TriggerClientEvent('ox_lib:notify', source, {title = 'Error', description = Lang:t('error.error'), type = 'error', duration = 5000 })
                    return
                end
            end)
        else
            TriggerClientEvent('ox_lib:notify', source, {title = 'Error', description = Lang:t('error.error_cid'), type = 'error', duration = 5000 })
            return
        end
    end)
end)
...
AddEventHandler("rsg-vendor:server:vendorName", function(location, name)
    ...
    exports.oxmysql:execute('SELECT * FROM market_owner WHERE marketid = ? AND citizenid = ?',{location, Playercid} , function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent('ox_lib:notify', source, {title = 'Success', description = Lang:t('success.newname'), type = 'success', duration = 5000 })
        else
            TriggerClientEvent('ox_lib:notify', source, {title = 'Error', description = Lang:t('error.error'), type = 'error', duration = 5000 })
            return
        end
...
AddEventHandler("rsg-vendor:server:vendorRob", function(location)
    ...
    exports.oxmysql:execute('SELECT * FROM market_owner WHERE marketid = ?',{location} , function(result)
        ...
        if result[1].money >= 1 then
            ...
            TriggerClientEvent('ox_lib:notify', source, {title = 'Success', description = Lang:t('success.robreward')..rpmoney, type = 'success', duration = 5000 })
        else
            TriggerClientEvent('ox_lib:notify', source, {title = 'Error', description = Lang:t('error.market_no_money'), type = 'error', duration = 5000 })
            return
        end
    end)
end)
